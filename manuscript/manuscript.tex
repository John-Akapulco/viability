...
routes were compared: a composition-based heuristic combining elemental
identity with the \texttt{is\_metal} flag, and a bonding-based
classification built directly from the LOBSTER output of
Section~\ref{sec:icohp-icobi-def} (\texttt{is\_metal} for metallic
character; otherwise the mean ICOBI of the dominant, first-coordination-shell
species pair, thresholded at the midpoint between the ionic and covalent
medians of the observed distribution, separates ionic from covalent
character, with compounds combining both a strongly ionic and a strongly
covalent/homoatomic dominant pair --- the Zintl-type signature --- assigned
to the mixed category). First-coordination shells, where used (this
classification and the validation reactions of
Section~\ref{sec:validation-methods} only), are delimited by an adaptive,
self-calibrating rule on the sorted bond-distance spectrum of each species
pair (cutting at the first gap exceeding 2.5$\times$ the largest gap seen
so far within the putative shell), rather than by a fixed distance cutoff.
