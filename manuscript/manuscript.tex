...
tested independently: Spearman $\rho$ between the raw antibonding
population and each polymorph's enthalpy above its own group's most
stable member ($\Delta H$), and, where a case-1 reaction is available for
enough of the group, the same test against the reaction
$\Delta$(ICOHP\textsubscript{antibonding}).

\begin{table}[htbp]
\centering
\caption{Spearman correlation between the antibonding descriptor and
...
\toprule
& \multicolumn{3}{c}{Raw antibonding} & \multicolumn{3}{c}{$\Delta$(ICOHP\textsubscript{antibonding}), reaction} \\
Group & $n$ & $\rho$ & $p$ & $n$ & $\rho$ & $p$ \\
\midrule
Ionic (25 formulas) & 53 & $-0.089$ & $0.53$ & 48 & $-0.008$ & $0.96$ \\
Covalent (6 formulas) & 20 & $-0.206$ & $0.38$ & 8 & $-0.172$ & $0.68$ \\
Mixed (10 formulas) & 24 & $+0.079$ & $0.72$ & 9 & $-0.322$ & $0.40$ \\
Metallic (27 formulas) & 62 & $+0.114$ & $0.38$ & 39 & $-0.207$ & $0.21$ \\
\bottomrule
\end{tabular}
\end{table}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.85\textwidth]{polymorph_antibonding_correlation.png}
\caption{Polymorph enthalpy above the same-formula ground state versus
raw ICOHP antibonding population, one panel per \texttt{icobi\_label}
bond-type group.}
\label{fig:polymorph-antibonding}
\end{figure}

No group reaches significance, for either the raw or the reaction
antibonding construction ($p>0.2$ throughout; ionic's raw-descriptor
result, the closest, is $p=0.53$ pooled and $p=0.068$ restricted to
non-ground-state members only, still short of the conventional
threshold). This mirrors the fully integrated reaction $\Delta$(ICOHP)'s
own Case 2 result exactly: whatever makes
$\Delta$(ICOHP\textsubscript{antibonding}) an effective viability
pre-screen against a compound's own elements
