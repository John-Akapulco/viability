...
assuming one mirrors the other. A fifth group, \emph{elemental}
(single-element allotropes), is pooled across every
\texttt{icobi\_label} value rather than restricted to one, since pure
elements are not always classified consistently across their own
allotropes by that scheme (e.g.\ graphite alone is \texttt{metallic} via
\texttt{is\_metal}, while every other carbon allotrope is
\texttt{covalent}); only 3 elements (C, Sn, Zn) have $\geq2$ allotropes
in the current dataset, and none has a case-1 reaction of its own (an
element cannot decompose into ``other elements''), so only the raw
construction is tested for this group.
...
stable member), by bond-type group.}
...
Elemental (3)$^\dagger$ & 10 & $-0.159$ & $0.66$ & --- & --- & --- & 10 & $-0.324$ & $0.36$ & --- & --- & --- \\
\bottomrule
\end{tabular}
\vspace{0.1cm}
\\{\footnotesize Number of same-formula groups per bond type in
parentheses. $^\dagger$3 formulas/10 rows only --- far below this
project's own $n<15$ underpowered-result convention; reported for
...
raw antibonding population, one column per bond-type group (elemental
pooled across \texttt{icobi\_label}), ICOHP-based (top row) and
ICOBI-based (bottom row).}
...
mixed's raw ICOBI result, $p=0.084$, $n=24$ formulas/rows only) ---
including the elemental group, though at $n=10$ it is too small to
weigh either way. This
