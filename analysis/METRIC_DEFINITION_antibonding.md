...
This is a genuinely different question from everything computed so far in this project: `connectivity_path.py`, `network_dimensionality.py`, and `periodic_mincut.py` all operate on **integrated** ICOHP/ICOBI (a single number per bond, already summed over all energies up to E_F) — none of them look at *how* COHP is distributed in energy, i.e. none can see whether the frontier states specifically are more or less antibonding than the average.
...
Two variants reported, following the same raw/normalized pattern already used for the connectivity weight and the min-cut descriptor:
...
- No claim that this metric predicts anything (stability, persistence, or otherwise) — that requires the same kind of correlation testing already applied to the connectivity weight and min-cut, against `energy_above_hull`/`formation_energy_per_atom` or (better, per mission #3's own conclusion) real persistence labels. Not done here.
...
*Code*: `cohp_extraction.py` (`frontier_reference_energy`, `integrate_antibonding_in_window`, `antibonding_population_near_frontier`), `connectivity_path.py` untouched. *Tests*: `tests/test_cohp_extraction.py` (14/14 passing; full project suite 34/34).
