"""Simple, interpretable statistics on analysis/connectivity_vs_hull.csv:
Spearman correlation (connectivity weight, raw and normalized, vs
...
CSV_PATH = HERE / "connectivity_vs_hull.csv"
...
CANDIDATE_METRICS = {
    "icohp_connectivity_weight_min": "connectivity weight (raw)",
    "icohp_connectivity_weight_min_normalized": "connectivity weight (normalized by |ICOHP_min|)",
    "icohp_sum": "ICOHP sum",
...
def make_scatter_figure(df: pd.DataFrame) -> Path:
    ...
    for bond_type, sub in df.groupby("bond_type"):
        sub = sub.dropna(subset=["icohp_connectivity_weight_min"])
        if sub.empty:
            continue
        ax.scatter(
            sub["energy_above_hull_eV_at"],
            sub["icohp_connectivity_weight_min"],
    ...
    ax.set_ylabel("Connectivity weight (eV, log scale)")
    ax.set_title("Connectivity weight vs. distance to the convex hull")
    ax.legend()
    fig.tight_layout()
    out = FIG_DIR / "connectivity_vs_ehull.png"
...
def logistic_reference_model(df: pd.DataFrame) -> dict:
    sub = df.dropna(subset=["icohp_connectivity_weight_min_normalized", "icohp_mean", "bond_type", "family"]).copy()
    ...
    X = pd.concat(
        [sub[["icohp_connectivity_weight_min_normalized", "icohp_mean"]].reset_index(drop=True),
...
def main():
    ...
    summary = {
        ...
        "n_disconnected_or_missing_connectivity": int(df["icohp_connectivity_weight_min"].isna().sum()),
