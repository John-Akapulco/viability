"""Join connectivity_path.py's output with each compound's Materials Project
metadata into a single flat table for statistical analysis.

Inputs:
  - mp_dataset/results.json           (connectivity_path.py output, all compounds)
...
Output: analysis/connectivity_vs_hull.csv
"""
...
OUT_CSV = Path(__file__).parent / "connectivity_vs_hull.csv"
...
def main():
    ...
    for r in rows:
        ...
        if result is None:
            n_missing_result += 1
            continue  # not yet processed by connectivity_path.py (job still running/pending/failed)
        if result.get("error"):
            continue  # connectivity_path.py itself flagged this compound as failed
        ...
        final_rows.append({
            ...
            "icohp_connectivity_weight_min": weight_min,
            "icohp_connectivity_weight_min_normalized": weight_min_norm,
            "icohp_connectivity_direction": icohp.get("min_direction"),
            "icohp_sum": icohp_agg.get("sum"),
            "icohp_mean": icohp_agg.get("mean"),
            "icohp_min": icohp_min,
            "icohp_max": icohp_agg.get("max"),
            "icobi_connectivity_weight_min": icobi.get("min_weight"),
    ...
    print(f"Wrote {len(df)} rows to {OUT_CSV} ({n_missing_result} compounds skipped: no connectivity_path.py result yet)")
