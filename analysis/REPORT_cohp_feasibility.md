...
For every bond label in `ICOHPLIST.lobster` (already validated throughout this project — it's what `connectivity_path.py` consumes), the ICOHP trace read from `COHPCAR.lobster` (independently, via `Cohpcar`) at the energy index closest to E=E_F was compared against the `ICOHPLIST.lobster` value for the same label:
...
- `connectivity_path.py` and its results untouched.
- No SISSO.

---

*New code*: `cohp_extraction.py` (repo root, `connectivity_path.py` untouched). *Data*: `analysis/yargla_file_survival.json`, `mp_dataset/structures_cohp/` (6 compounds, COHPCAR.lobster + ICOHPLIST.lobster + CONTCAR + metadata, no WAVECAR/COBICAR duplicated). *Tests*: `tests/test_cohp_extraction.py` (4/4 passing, real LOBSTER data).
