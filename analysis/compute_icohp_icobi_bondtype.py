...
Reuses analysis/connectivity_vs_antibonding.csv's per-compound icobi_mean
(mean ICOBI per bond, dimensionless bond order, already computed by
connectivity_path.py -- no LOBSTER re-parsing needed there) and is_metal
...
through connectivity_path.py), so its icobi_mean is computed directly
...
IN_CSV = REPO_ROOT / "analysis" / "connectivity_vs_antibonding.csv"
