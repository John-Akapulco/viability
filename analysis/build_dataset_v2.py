...
onto the existing connectivity_vs_hull.csv columns.
...
read. Reuses connectivity_path.load_bonds and the mission-#3 modules
(network_dimensionality, periodic_mincut) unchanged -- no duplicated
parsing or algorithm logic.

Output: analysis/connectivity_vs_formation_energy.csv
"""
...
sys.path.insert(0, str(REPO_ROOT / "Connectivity_viability"))
from connectivity_path import load_bonds, max_translation_extent  # noqa: E402
...
CONNECTIVITY_CSV = Path(__file__).parent / "connectivity_vs_hull.csv"
FORMATION_ENERGY_JSON = REPO_ROOT / "mp_dataset" / "formation_energies.json"
OUT_CSV = Path(__file__).parent / "connectivity_vs_formation_energy.csv"
...
def compute_row_extras(compound_id: str) -> dict:
    ...
    for metric in ("icohp", "icobi"):
        ...
        if metric == "icohp":
            ...
            # raw value is the strongest bond, matching connectivity_path.py's
...
def main():
    base = pd.read_csv(CONNECTIVITY_CSV)
