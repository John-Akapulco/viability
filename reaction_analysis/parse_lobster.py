...
examples/dataset/ toy compounds connectivity_path.py's own tests already
...
`connectivity_path.py`'s own graph builder adds the reverse-direction edge
...
pymatgen access pattern already established in connectivity_path.py's
...
def raw_bond_records(compound_dir: Path, filename: str, are_cobis: bool) -> list[dict]:
    ...
    pymatgen access pattern as connectivity_path.py's _ingest() -- see this
    module's docstring."""
