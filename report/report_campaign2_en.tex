...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{Campaign 2 --- Connectivity vs. Thermodynamic Stability}
\fancyhead[R]{\thepage}
\renewcommand{\headrulewidth}{0.4pt}

\title{\bfseries Connectivity Weight and Thermodynamic Stability \\
...
compounds, connectivity weight only) unmodified.
...
\smallskip
\small\itshape Update, August 16, 2026 (later the same day): the
project's central question reformulated around $\Delta$(ICOHP)-based
viability discrimination, with a formal manuscript concordance test and
an extension to every compound computed to date, pooled across all
campaigns --- see \S\ref{sec:reframing}. \texttt{connectivity\_path.py}
is now background/historical material, not the organizing question.
\end{center}
...
The first report (6 toy compounds) established that the connectivity
descriptor computed by \texttt{connectivity\_path.py} diverges sharply from
...
does the connectivity descriptor carry predictive information about
...
connectivity weight (raw and normalized by $|\text{ICOHP}_\text{min}|$) and
...
\node[box, below=of vasplob] (perc)   {\texttt{connectivity\_path.py} (unchanged) \\ 186 compounds (180 + 6 pilots)};
...
indistinguishable in strength. \textbf{The connectivity descriptor does not
...
Stable (on-hull) vs. metastable, features: normalized connectivity weight,
...
this sample size, the connectivity descriptor shows no significant
...
\noindent\textit{Data:} \texttt{analysis/connectivity\_vs\_hull.csv} \\
...
than anything the connectivity weight ever achieved
...
Connectivity weight, normalized (mission \#1) & 177 & 0.104 & 0.170 \\
\bottomrule
\end{tabular}
\end{center}
...
and endobondic/exobondic label, distinct from the 7 literature cases
...
endobondic/exobondic classification it supports (Reitz \& Dronskowski,
ic-2026-04181q) --- distinguish thermodynamically stable, metastable,
and unstable compounds?} This supersedes the connectivity-weight-centric
framing that the body of this report (\S\ref{sec:reframing} and earlier)
documents --- \texttt{connectivity\_path.py} (mission \#1) and
\texttt{network\_dimensionality.py}/\texttt{periodic\_mincut.py}
(mission \#3) remain valuable as the historical foundation (periodic
graph construction, the statistical conventions every later mission
still reuses) but are no longer this project's organizing question,
consistent with min-cut's own headline correlation not surviving the
dataset's growth to 349 compounds (\S\ref{sec:followups2}). Full
detail: \texttt{analysis/REPORT\_delta\_icohp\_viability.md},
\texttt{analysis/test\_delta\_icohp\_viability.py}.

\subsection{Manuscript concordance, formalized}

\S\ref{sec:followups2} already showed all 7 of Reitz \& Dronskowski's
worked reactions reproducing to within a few kJ/mol. Correlation alone
(Pearson/Spearman) is not the right test for ``do our numbers match the
manuscript's'' --- it is satisfied by any monotonic relationship, even
one with a large systematic offset or scale factor. \textbf{Lin's
Concordance Correlation Coefficient} (CCC) specifically tests agreement
with the identity line (computed = manuscript), the actual claim being
tested:
...
manuscript's own reporting precision (values rounded to 2--3 decimals in
the original text), not merely that they move together --- the
implementation is validated by it, not just individually spot-checked.
...
\S\ref{sec:followups2}'s min-cut diagnosis already demonstrated).
\textbf{281 case-1 reactions}, every element and compound with a
computed decomposition-into-elements reaction across the whole project,
...
expected direction, given endobondic's manuscript meaning as a kinetic-
barrier signature --- but not significant at this sample size).
...
despite sitting off the convex hull --- the category the manuscript's
own framework is specifically about) have the highest endobondic
fraction; \texttt{theo\_metastable} compounds (never synthesized) have
the lowest.

\subsection{Reading}

The manuscript's 7 cases reproduce essentially exactly --- the
...
right way. This is not a contradiction: the manuscript's 7 cases were
...
into elements is strongly endothermic for nearly every compound (normal
...
(\texttt{icohp\_connectivity\_weight\_min}, this work's main metric).
Auto-generated from \texttt{analysis/connectivity\_vs\_hull.csv} by
...
\texttt{download\_extension4.py}): the 62-element decomposition reference
