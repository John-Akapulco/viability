...
connectivity weight, the original campaign 2 subject. Everything
concerning the connectivity weight and its direct descendants (network
dimensionality, periodic min-cut --- missions \#1 and \#3) has been moved
to Appendix~\ref{sec:appendix-connectivity}, ``Connectivity\_viability,''
with the corresponding source code relocated to the repository's
\texttt{Connectivity\_viability/} directory. The main body below documents
...
compounds) established that a connectivity descriptor diverges sharply
...
whose descriptor itself (the connectivity weight) and its own result are
now documented in Appendix~\ref{sec:appendix-connectivity}, not in this
...
The connectivity weight (\texttt{Connectivity\_viability/connectivity\_path.py})
...
Appendix~\ref{sec:appendix-connectivity}.
...
for every later mission. \textbf{The connectivity weight, which appeared
in this same table in the original version of this report, is moved in
full to Appendix~\ref{sec:appendix-connectivity}} (complete table with
...
connectivity-vs-classic-aggregates comparison; that comparison is still
...
Limits specific to the connectivity weight and its descendants
(primitive-vs-conventional-cell sensitivity, chance-level reference
logistic regression) are documented in
Appendix~\ref{sec:appendix-connectivity}, not repeated here.
...
way: starting from the connectivity weight (no significant correlation
retained, Appendix~\ref{sec:appendix-connectivity}), through network
...
than anything the connectivity weight or the min-cut ever achieved
(Appendix~\ref{sec:appendix-connectivity}) --- but in both cases that
...
connectivity weight (mission \#1, $\rho=0.104$), on the same $n=177$, are
given for reference in Appendix~\ref{sec:appendix-connectivity}.}
\end{center}
...
Appendix~\ref{sec:appendix-connectivity}, not here.)
...
supersedes the connectivity-weight-centric framing that organized the
original campaign 2 (Appendix~\ref{sec:appendix-connectivity}) ---
connectivity weight (mission \#1) and network dimensionality/periodic
...
Appendix~\ref{sec:appendix-connectivity}). Full detail:
...
Appendix~\ref{sec:appendix-connectivity}'s min-cut diagnosis already
