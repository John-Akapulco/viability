...
methodology (`Connectivity_viability/connectivity_path.py`, mission #1;
`Connectivity_viability/network_dimensionality.py`/
`Connectivity_viability/periodic_mincut.py`, mission #3) established the
...
central question, so it now lives in its own `Connectivity_viability/`
...
3. **[Appendix: original graph-based methodology](#appendix-original-graph-based-methodology-connectivity-dimensionality-min-cut--missions-1-and-3)** — `Connectivity_viability/connectivity_path.py` (mission #1) and `Connectivity_viability/network_dimensionality.py`/`Connectivity_viability/periodic_mincut.py` (mission #3). Historical foundation (periodic graph construction, statistical conventions); no longer the project's central question, and min-cut's own headline correlation did not survive the dataset's growth to 349 compounds (diagnosed, not just noted — see the appendix).
...
natural extension not yet implemented — `connectivity_path.py` already
treats ICOHP and ICOBI symmetrically as alternative edge weights, but
`cohp_extraction.py` is COHP/ICOHP-only for now.

---

## Appendix: original graph-based methodology (connectivity, dimensionality, min-cut — missions #1 and #3)
...
different physical question from the connectivity weight: separability, not
...
than the original connectivity weight ever achieved — though likely driven
...
### `connectivity_path.py` — the original descriptor (mission #1)
...
[Analysis](#analysis-of-connectivity_pathpy-missions-1-2) — but the graph
...
```bash
python Connectivity_viability/connectivity_path.py --root dataset --metric icohp --output results.csv
python Connectivity_viability/connectivity_path.py --root dataset --metric icohp,icobi --output results.csv --also-json results.json
python Connectivity_viability/connectivity_path.py --root dataset --metric icohp --bond-pair Fe-O --output results.csv
```
...
- `*_connectivity_weight_min`: weight of the weakest connectivity path,
  across all directions — the main descriptor.
- `*_connectivity_direction`: the corresponding direction (`a`, `b`, or `c`).
- `*_connectivity_weight_a/b/c` and `*_connectivity_status_a/b/c`: minimum
...
- `*_sum`, `*_mean`, `*_min`, `*_max`: classic aggregates over the same
  bonds, for direct comparison against the connectivity descriptor.
...
2. For each lattice direction `a`, `b`, `c`, the minimum-weight path in
   ...
   by `--coord-bound`.
3. The minimum over starting atoms gives that direction's connectivity
...
```bash
python Connectivity_viability/connectivity_path.py --root examples/dataset --metric icohp --output /tmp/example_results.csv -v
```

#### Analysis of `connectivity_path.py` (missions #1-#2)

`analysis/` contains a statistical study of the connectivity descriptor
...
normalized connectivity weight vs. the classic ICOHP aggregates, a reference
...
Pipeline: `analysis/build_dataset.py` (join `connectivity_path.py` output +
MP metadata → `analysis/connectivity_vs_hull.csv`) then
...
choice does change the connectivity weight substantially for compounds
...
#### Known limitations (`connectivity_path.py` CLI)
