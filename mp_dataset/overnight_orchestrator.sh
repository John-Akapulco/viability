...
# finish), then runs connectivity_path.py + the analysis join/stats scripts.
...
echo "[orchestrator] running connectivity_path.py over mp_dataset/structures/"
python connectivity_path.py --root mp_dataset/structures --metric icohp,icobi \
  --output mp_dataset/results.csv --also-json mp_dataset/results.json -v \
  > mp_dataset/connectivity_path_run.log 2>&1
echo "[orchestrator] connectivity_path.py exit=$?"
